{ pkgs }:
{
  ...
  llm-ocr-area = pkgs.writeShellScriptBin "llm-ocr-area" ''
    ...
    selection="$(${pkgs.hacksaw}/bin/hacksaw -f "-i %i -g %g")"
    ${pkgs.shotgun}/bin/shotgun $selection - | ${pkgs.llm}/bin/llm -a - "Please output your best guess at the Japanese characters in this image. Do not output any other text." | ${pkgs.xclip}/bin/xclip -selection clipboard
  '';
